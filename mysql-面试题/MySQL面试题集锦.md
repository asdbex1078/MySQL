...
* [（四）索引方面的问题](#%E5%9B%9B%E7%B4%A2%E5%BC%95%E6%96%B9%E9%9D%A2%E7%9A%84%E9%97%AE%E9%A2%98)
  ...
  * [22\. 多列索引中为什么范围匹配之后，索引失效？](#22-%E5%A4%9A%E5%88%97%E7%B4%A2%E5%BC%95%E4%B8%AD%E4%B8%BA%E4%BB%80%E4%B9%88%E8%8C%83%E5%9B%B4%E5%8C%B9%E9%85%8D%E4%B9%8B%E5%90%8E%E7%B4%A2%E5%BC%95%E5%A4%B1)
...
## 10.COMPACT行格式中，为什么 变成字符串和null值列表需要逆序排列？
...
4. 平均值组大小对索引选取的影响
5. 成本的选择，放弃了最优的索引
6. innodb提供的统计信息不准确，造成优化器选取了错误的索引

---

## 22. 多列索引中为什么范围匹配之后，索引失效？

多列索引，按照聚蔟索引 和 非聚簇索引来分类，他归为 非聚簇索引。按照 索引 的建立顺序进行排列。

判断能不能走索引，看他是不是一次 I/O。

以下图片所示，建立的 a_b_c 索引，保持先按照 a 从小到大进行排列，a相等，则按照 b 从小到大排列，a 、b 都相等，则按照 c 从小到大排列。

![image-20210523182204022](../mysql-image/联合索引之范围查询索引失效1.png)

当我需要查询 select * from table where a = 5 and b > 1 and c = 1 时，查询数据时，**存储引擎查找数据所经历的步骤**为：

第一步：查找到 a = 5 的数据，找到7条满足条件的 行：

![image-20210523182412714](../mysql-image/联合索引之范围查询索引失效2.png)

第二步：查找b > 1 的数据，过滤掉前两行，剩下满足条件的 有 5 行

![image-20210523182513499](../mysql-image/联合索引之范围查询索引失效3.png)

第三步：找 c = 1 的数据，此时存储引擎发现，**c = 1 是散落开的，并不能通过一次 IO，将 c = 1 的所有数据找到。**

![image-20210523182647188](../mysql-image/联合索引之范围查询索引失效4.png)

所以此时，就保持 让 a b 能够走索引，c 是无法走索引的。

由于他是非聚簇索引，叶子节点中会存放主键id值。存储引擎根据 a = 5 and b > 1的 5 条数据的主键id **回表查询**，将查出来的 5 条数据的全部内容 交给 server 层，让 server 层 去过滤掉 c != 1 的数据，最终返回给客户端 3 条数据。

> 额外延伸：MySQL 5.6 版本新增了 ICP 功能，专门针对多列索引做出的优化。
>
> 面对以上问题的时候，c 可以走 ICP，条件下推到存储引擎层：即 server 层 把过滤掉    c != 1 的条件交给存储引擎。这样存储引擎就可以过滤掉 c != 1 的数据，返回给 server 层 3条数据，这3条数据就是最终的数据。
>
> 好处：存储引擎返回 server 层的数据更少了，消耗的内存会少点。
>
> 虽然是走了 ICP，但严格意义上来说 b 的范围查询后，导致 c 失效了，ICP的出现，提升了这个方面的性能损失。
...
正确地创建和使用索引是实现高性能的基础，索引的创建都是需要建立在一定基础上的，并不能随便建立索引。以下会列出几点，如何创建合适的索引。

1. 第一点，最重要的一点，一定要结合业务场景，使用情况建立索引。有些索引是可以建表期间确定的，比如订单号，身份证号他们能确保唯一性。
2. 列值的类型注意点：主键、唯一列**禁止使用 UUID**，一般这两个都是需要建立索引的，UUID会导致频繁的页分裂，消耗内存。
3. **选择性低的列不适合建立单列索引**，重复值太多，会造成大量回表，甚至优化器会放弃使用该索引，例如不适合给性别建立单列索引。
4. **不常用的列不适合建立索引**，建立索引太多会导致插入删除更新时额外的维护索引列，造成性能问题
5. 结合上一点，不常用的列，但偶尔用到的列，可以**考虑建立联合索引，减少单列索引的数量**
6. **尽量使用覆盖索引**，即索引树中包含所有的返回结果，避免回表。
7. 视业务情况，数据情况，将**选择性极低的列**放前面，比如 第25题，将性别放在最前面，可以巧妙地利用 in 查询走索引
8. 多个索引中，**尽量按照选择性高的列**放前面，且这个选择性高的使用频率 大于 他后边的列的使用频率
9. **需要按照范围排序的字段放最后**，因为范围查询后索引会失效。(为什么？)
10. **连表查询时，on 条件里的列建立索引，加快扫描表的速度。**

>案例一：一个商品表中，经常查询的字段是商品关键词，发布时间，访问量，价格，如何建立合适的索引？
>
>结合我们平时购物的场景，肯定想花最少的钱，买最好的商品。假设我们正在使用京东APP，怎么进行搜索的？
>
>- 方案一：
>  1. 首先得有关键词，按这个关键词去搜索商品，所以商品关键词放第一位。
>  2. 然后是该商品的受欢迎程度，这种商品一般都说明比较好，所以可以将访问量放第二位，降序排列。
>  3. 接下来考虑的是价格，按照价格从低到高排序，此时按照 关键词、访问量、价格 排序之后，放在最前面的就是热销价格最低的商品。
>
>- 方案二：当然不排除有些人比较特殊，他不看是否热销，只看价格，所以可以建立 关键词、价格、访问量 索引
>
>如果考虑发布时间的话，可以在以上两条索引的最后加上 发布时间，因此可以查指定时间之前或之后的商品。
...
```shell
更新日志：
20210409：新增一道问题，COMPACT行格式中，为什么 变成字符串和null值列表需要逆序排列？
20210512：增加索引失效的场景
20210523：范围查询后为什么索引会失效？
```
