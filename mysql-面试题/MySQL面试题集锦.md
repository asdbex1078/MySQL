...
由于他是非聚簇索引，叶子节点中会存放主键id值。存储引擎根据 a = 5 and b > 1的 5 条数据的主键id **回表查询**，将查出来的 5 条数据的全部内容 交给 server 层，让 server 层 去过滤掉 c != 1 的数据。

> 额外延伸：MySQL 5.6 版本新增了 ICP 功能，这个时候，c 可以走 ICP，条件下推到存储引擎层：即 server 层 把过滤掉    c != 1 的条件交给存储引擎。这样存储引擎就可以过滤掉 c != 1 的数据。
>
